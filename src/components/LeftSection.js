...
export default function LeftSection() {
    ...
    return (
        <div className='w-[18%] mr-2  font-sans h-screen'>
                ...
                {svgCollection.map((svg) => {
                    return (
                        <li key={svg + Math.random() * 11}>
                            <a href="/" className='flex justify-start space-x-5  hover:bg-gray-800 hover:rounded-3xl p-2 focus:font-bold'> {svg}  </a>
                        </li>
                    )
                })}
            ...
            <div > <a href="/" className="block hover:bg-slate-200 focus:font-extrabold p-2 my-2 text-black bg-white rounded-3xl text-center font-bold text-lg">Post</a></div>

            <div >
                <a href="/" className="flex justify-evenly align-middle hover:bg-gray-800  p-2 pr-3 my-2  hover:rounded-full fixed bottom-1">
